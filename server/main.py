...
obstacle_dist['very_close'] = fuzz.trapmf(obstacle_dist.universe, [0, 0, 50, 80])
obstacle_dist['close']      = fuzz.trimf(obstacle_dist.universe, [60, 100, 140])
obstacle_dist['far']        = fuzz.trapmf(obstacle_dist.universe, [110, 150, 500, 500])
...
# Make 'ahead' very narrow to avoid misclassifications of off-center obstacles
obstacle_angle['left']  = fuzz.trapmf(obstacle_angle.universe, [-180, -180, -5, 0])
obstacle_angle['ahead'] = fuzz.trimf(obstacle_angle.universe, [-2, 0, 2])
obstacle_angle['right'] = fuzz.trapmf(obstacle_angle.universe, [0, 5, 180, 180])
...
rules = [
    ...
    # When close or very close, ignore the goal and just dodge away from the obstacle!
    ...
    # Dead ahead obstacle -> tie breaker: dodge left consistently
    ctrl.Rule(obstacle_dist['very_close'] & obstacle_angle['ahead'], steering['hard_left']),
    ctrl.Rule(obstacle_dist['close'] & obstacle_angle['ahead'],      steering['left']),
]
...
def steer_car(state: CarState):
    try:
        ...
        for obs in state.obstacles:
            ...
            # Calculate lateral and longitudinal distances to the obstacle
            rel_angle_rad = math.radians(rel_angle)
            x_lon = dist * math.cos(rel_angle_rad)
            y_lat = dist * math.sin(rel_angle_rad)

            # Threat Corridor Filtering:
            # 1. Obstacle is not completely behind our rear axle (x_lon > -35)
            # 2. Obstacle lies within our lateral path + safety margin (abs(y_lat) < 65)
            if x_lon > -35 and abs(y_lat) < 65:
